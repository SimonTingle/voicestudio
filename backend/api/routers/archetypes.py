...
def _preview_source(a: dict) -> tuple[str, str]:
    ...
    if _no_voice_model_downloaded():
        return "no_model", (
            ...
            "the engine's Weights list in Model Catalogue → Download."
        )
    ...
    if not is_playable_wav(cache_path):
        try:
            ...
            if _no_voice_model_downloaded():
                detail = (
                    "You're offline and no voice model is downloaded yet — "
                    "the engine's Weights list in Model Catalogue → Download. (Or turn on pre-rendered "
                    "voice previews in the engine's Weights list in Model Catalogue.)"
                )
            else:
                detail = (
        ...
        try:
            ...
            if _no_voice_model_downloaded():
                detail = (
                    ...
                    "downloaded yet. the engine's Weights list in Model Catalogue → Download."
                )
            else:
                detail = (
