...
- **Explicit choices are never auto-switched.** A user with an endpoint
  configured anywhere (the engine's Weights list in Model Catalogue, ``HF_ENDPOINT`` env, the
...
def explicit_endpoint():
    ...
    the engine's Weights list in Model Catalogue persists via user_env and what main.py loads at boot)
