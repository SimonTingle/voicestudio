...
- **Dictation no longer goes quiet when the chosen model can't actually transcribe.** The recommended dictation model (`sherpa-parakeet-tdt-v3`) installs cleanly, loads without a single error, and then returns *nothing* for perfectly clear speech on Windows — a defect inside sherpa-onnx's NeMo-TDT decoder that OmniVoice can't fix by configuration (reported upstream as [k2-fsa/sherpa-onnx#3767](https://github.com/k2-fsa/sherpa-onnx/issues/3767)). Dictation just appeared broken. Now, when a session hears real speech but the model returns no text, OmniVoice finishes the job on its regular speech-recognition engine so you still get your words, and tells you which model let you down. It also remembers, so the next session skips straight to the engine that works instead of repeating the dead end — and picking that model again in Settings gives it a fresh chance, for when a future sherpa-onnx release fixes it. Saying nothing still produces nothing: only a model that stays silent through actual speech is demoted. (#1175)

- **The dictation bubble can no longer get stuck on screen.** A dictation session that failed with nothing to show for it — microphone unavailable, model missing, the backend unreachable — left the floating pill parked on top of everything, forever, with no way to dismiss it but restarting. Failures that *do* hold a transcript still stay up (that text is yours to copy), but a failure with nothing to rescue now clears itself. (#1175)
...
- **Closing the app (or any shutdown) while the model is loading no longer reports a scary "crash".** transformers loads the TTS weights in its own thread pool; when the backend is torn down mid-load — the user closes the window during first-load, uvicorn is told to stop, or a port bind fails — that pool raises `RuntimeError: cannot schedule new futures after interpreter shutdown`, which was logged as `Model loading failed` with a full traceback and flipped the model status to a phantom error. The loader now recognises an interpreter-shutdown teardown for what it is and logs it calmly (walking the wrapped cause chain transformers buries it under), so an ordinary shutdown-during-load doesn't masquerade as a crash. (#1183)

- **The backend-crash notice is no longer hidden behind the top navbar.** When the backend process died, the "backend crashed — View crash details" banner was pinned to the very top of the window at a lower stacking level than the navbar, so the navbar painted over it and the **"View crash details" button wasn't clickable** (or even visible). The banner now sits just below the navbar and above its stacking level, so the whole alert and its actions are always reachable. (#1182)

- **Building from source on Windows: `bun desktop` no longer crashes in `setup:api` with a `UnicodeEncodeError`.** The post-install `scripts/setup.py` prints `✓`/`⚙` status glyphs, but when its stdout is a pipe (as it is under `bun run setup:api`, or in CI) Windows Python defaults to the `cp1252` codepage, which can't encode them — so the script died mid-setup and took `bun desktop` down with it. It only appeared to work in an interactive terminal by luck of the console's encoding. The script now forces UTF-8 on its own streams, so output is identical whether run interactively or piped. (#1181)

- **The Windows app no longer boots to a black screen.** On the released build the first-run splash crashed the instant it rendered — a blank, dark window and nothing else — because a production-only minifier quirk reordered a splash variable ahead of the `useState` that defines it (`Cannot access 'logs' before initialization`). It only ever bit the *minified* release bundle, so dev runs and the e2e suite (which exercise the un-minified dev server) never saw it; #1159 fixed the source-visible symptom but left the reorder hazard in place, so it still shipped. Every hook in the splash is now declared before the values derived from it, leaving the minifier nothing to reorder, and a regression test locks the ordering in. (#1178)

- **No more storm of black console windows on Windows — during setup *or* everyday use.** OmniVoice shells out constantly: first-run bootstrap runs `uv` / managed-Python / capability probes, and in normal use the backend spawns ffmpeg (dubbing), engine sidecars, `yt-dlp`, `demucs`, and ASR helpers. Because the app runs console-less, on Windows each of those console subprocesses got its own `cmd` window flashed on screen — a fresh install looked swarmed by pop-up terminals, and generating or dubbing kept flickering more. Both sources are now fixed: the Rust bootstrap routes every subprocess through one windowless chokepoint, and the Python backend patches `subprocess.Popen` at startup so **every** child — including ones spawned deep inside third-party libraries (`imageio-ffmpeg`, `yt-dlp`) that we can't edit — runs with `CREATE_NO_WINDOW`. All output still streams into the app's own log panel. macOS and Linux are unaffected. (#1178)

- **Building from source: `bun desktop` no longer fails with a cryptic "cargo … program not found" on a stale terminal.** `tauri dev` shells out to `cargo`, so a terminal opened before the Rust toolchain was installed — holding a `PATH` snapshot without `~/.cargo/bin` — broke the desktop dev launcher even though cargo was installed and on the persisted user `PATH`. The launcher now self-heals: it prepends `~/.cargo/bin` for the run when cargo isn't already visible (no-op when it is), and prints an actionable install hint if Rust genuinely isn't present. Contributor-facing only; no effect on shipped builds. (#1180)
